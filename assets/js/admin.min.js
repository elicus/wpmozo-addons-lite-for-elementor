!(function (e) {
    ...
    e(function () {
            ...
            if ( e('body').find('.wpmozo_ale_team_member_skills').length > 0 ) {
                e('body').on('blur', '.wpmozo_ale_team_member_skills', function(){
                    let skills = [];
                    e('.wpmozo_ale_team_member_skills').each(function(){
                        let value = e(this).val();
                        if ( value ){
                            skills.push( e(this).val() );
                        }
                    });
                    e('#wpmozo_ale_team_member_skills').val(skills);
                });
            }

            /* Fill metafields value in hidden field on blur */
            if ( e('body').find('.wpmozo_ale_team_member_skills_value').length > 0 ) {
                e('body').on('blur', '.wpmozo_ale_team_member_skills_value', function(){
                    let skills_val = [];
                    e('.wpmozo_ale_team_member_skills_value').each(function(){
                        let value = e(this).val();
                        if ( value ) {
                            skills_val.push( e(this).val() );
                        }
                    });
                    e('#wpmozo_ale_team_member_skills_value').val(skills_val);
                });
            }
            /* Add Row */
            e('body').on('click', '.wpmozo_repeator_meta_field_add_row', function(){
                let row  = '<div class="wpmozo_repeator_meta_field_row">';
                    ...
                    row += '<input type="text" class="wpmozo_ale_team_member_skills" placeholder="Skill" />';
                    row += '<input type="number" class="wpmozo_ale_team_member_skills_value" placeholder="Skill Value Between 0 to 100" step="1" min="0" max="100"/>';
                    row += '</div>';
            ...
            e('body').on('click', '.wpmozo_repeator_meta_field_remove_row', function(){
                ...
                e('.wpmozo_ale_team_member_skills').trigger('blur');
                e('.wpmozo_ale_team_member_skills_value').trigger('blur');
            });
    });
})(jQuery);
