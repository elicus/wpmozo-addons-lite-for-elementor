...
if ( ! class_exists( 'WPMOZO_AE_Before_After_Slider' ) ) {
	class WPMOZO_AE_Before_After_Slider extends Widget_Base {
		...
		protected function render() {
			...
			$order_class          = 'elementor-element-' . esc_html( $this->get_id() );

			if ( 'yes' === $settings['before_label_show_switcher'] ) {
				$before_label = ( '' !== $settings['before_label_text'] ) ? sprintf( esc_attr( '%s' ), esc_attr( $settings['before_label_text'] ) ) : '';
			} else {
				$before_label = '';
			}

			if ( 'yes' === $settings['after_label_show_switcher'] ) {
				$after_label = ( '' !== $settings['after_label_text'] ) ? sprintf( esc_attr( '%s' ), esc_attr( $settings['after_label_text'] ) ) : '';
			} else {
				$after_label = '';
			}
			?>
				...
				<script type="text/javascript">
					jQuery(function($) {
						$(".<?php echo esc_html( $order_class ); ?> .wpmozo-ale-before-after-image-wrapper").twentytwenty({
							default_offset_pct: "<?php echo esc_html( $offset ); ?>",
