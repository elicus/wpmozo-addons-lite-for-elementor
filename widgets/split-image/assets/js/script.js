( function ( $ ) {
    $( window ).on( "elementor/frontend/init", function () {
        var WPMOZOSplitImage = elementorModules.frontend.handlers.Base.extend( {
            bindEvents: function () {
                this.change();
            },
            change: function () {
                ...
                function wpmozo_init_split_image( thisObj ) {
                    const $w = thisObj.find( '.wpmozo_split_image_wrapper' ),
                        cols = parseInt( $w.data('columns') ) || 3,
                        image = $w.data('image'),
                        rows = parseInt( $w.data('rows') ) || 3;
                    ...
                    $w.find('.wpmozo_split_image_portion').css({
                        ...
                        // correct calculation
                        const posX = (x * 100) / (cols - 1 || 1);
                        const posY = (y * 100) / (rows - 1 || 1);
        ...
        elementorFrontend.elementsHandler.attachHandler( "wpmozo_ae_split_image", WPMOZOSplitImage );
    } );
} )( jQuery );
