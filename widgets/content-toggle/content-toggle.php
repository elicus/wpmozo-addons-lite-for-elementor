...
if ( ! class_exists( 'WPMOZO_AE_Content_Toggle' ) ) {
	class WPMOZO_AE_Content_Toggle extends Widget_Base {
		 ...
		 * @since    1.0.0
		 * @access   private
		 * @var      Array    $toggle_one_page_ids    Maintains a list of all page ids required in toggle 1.
		 */
		 ...
		 * @since    1.0.0
		 * @access   private
		 * @var      Array    $toggle_two_page_ids    Maintains a list of all page ids required in toggle 2.
		 */
						...
						<div class="wpmozo_content_one_toggle animate__animated animate__<?php echo esc_attr( $settings['entrance_animation'] ); ?> wpmozo_content_active wpmozo_content_container wpmozo_content_toggle_<?php echo esc_attr( $toggle_one_type ); ?>">
							...
							if ( 'page' === $toggle_one_type ) {
								...
								if ( 0 === $toggle_one_page_id ) {
									$toggle_one_content = esc_html__( 'Please select a page.', 'wpmozo-addons-for-elementor' );
								} else {
									...
									if ( in_array( $toggle_one_page_id, self::$toggle_one_page_ids, true ) && isset( self::$toggle_one_page_ids[1] ) ) {
										$toggle_one_content = esc_html__( 'Recursion found! Ensure ', 'wpmozo-addons-for-elementor' ) . esc_html( get_the_title( self::$toggle_one_page_ids[0] ) ) . esc_html__( ' page doesn\'t include this page in toggle one', 'wpmozo-addons-for-elementor' );
									} else {
								...
								if ( 0 === intval( $toggle_one_template_id ) ) {
									$toggle_one_content = esc_html__( 'Please select a template.', 'wpmozo-addons-for-elementor' );
								} else {
						...
						<div class="wpmozo_content_two_toggle animate__animated animate__<?php echo esc_attr( $settings['entrance_animation'] ); ?>  wpmozo_content_container wpmozo_content_toggle_<?php echo esc_attr( $toggle_two_type ); ?>">
							...
							if ( 'page' === $toggle_two_type ) {
								...
								if ( 0 === $toggle_two_page_id ) {
									$toggle_two_content = esc_html__( 'Please select a page.', 'wpmozo-addons-for-elementor' );
								} else {
									$elementor_instance = Plugin::$instance;

									// Get and render the Elementor content.
									if ( in_array( $toggle_two_page_id, self::$toggle_two_page_ids, true ) && isset( self::$toggle_two_page_ids[1] ) ) {
										$toggle_two_content = esc_html__( 'Recursion found! Ensure ', 'wpmozo-addons-for-elementor' ) . esc_html( get_the_title( self::$toggle_two_page_ids[0] ) ) . esc_html__( ' page doesn\'t include this page in toggle two', 'wpmozo-addons-for-elementor' );
									} else {
								...
								if ( 0 === intval( $toggle_two_template_id ) ) {
									$toggle_two_content = esc_html__( 'Please select a template.', 'wpmozo-addons-for-elementor' );
								} else {
									$plugin_elementor   = Plugin::instance();
