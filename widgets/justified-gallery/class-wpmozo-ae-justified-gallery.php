...
/**
 *
 * Defines the Justified Gallery widget for WPMozo, allowing users to create responsive image galleries within Elementor.
 *
...
if ( ! class_exists( 'WPMOZO_AE_Justified_Gallery' ) ) {
	/**
	 * Class WPMOZO_AE_Justified_Gallery
	 *
	 * This class extends the Widget_Base class and is responsible for rendering the Justified Gallery widget in the WPMozo plugin.
	 * It includes methods for initializing the widget, rendering the gallery, and handling various settings and options for the gallery display.
	 *
	 * @package WPMOZO Lite
	 */
	class WPMOZO_AE_Justified_Gallery extends Widget_Base {
		...
		public function get_style_depends() {
			wp_register_style( 'wpmozo-ae-justified-gallery', plugins_url( 'assets/css/style.min.css', __FILE__ ), null, WPMOZO_ADDONS_FOR_ELEMENTOR_VERSION );
			return array( 'wpmozo-ae-justified-gallery-global-style', 'wpmozo-ae-justified-gallery', 'wpmozo-ae-magnific-popup-style' );
		}
		...
		public function get_script_depends() {
			wp_register_script( 'wpmozo-ae-justified-gallery', plugins_url( 'assets/js/script.min.js', __FILE__ ), array( 'jquery' ), WPMOZO_ADDONS_FOR_ELEMENTOR_VERSION, true );
			wp_localize_script(
				'wpmozo-ae-justified-gallery',
				'ajax_object',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'wpmozo_ae_justified_gallery' ),
				)
			);
			return array( 'wpmozo-ae-justified-gallery-global', 'wpmozo-ae-justified-gallery', 'wpmozo-ae-imagesloaded', 'wpmozo-ae-twbspagination' );
		}
		...
		protected function register_controls() {
			...
			include_once plugin_dir_path( __DIR__ ) . 'justified-gallery/assets/controls/controls.php';
		}

		/**
		 * Process the font icon.
		 *
		 * This method processes a font icon for the image overlay by splitting the provided icon string
		 * into its components (unicode, library, and weight), and generating the appropriate HTML for
		 * the font icon based on the library (e.g., Font Awesome).
		 *
		 * @since   1.3.0
		 * @access  public
		 * @package WPMOZO Lite
		 * @return  string HTML string for the font icon, or an empty string if the input is invalid.
		 * @param   string $icon_string The font icon string in the format 'unicode||library||weight'.
		 */
		...
		protected function render() {
							...
							?>
									<li <?php $this->print_render_attribute_string( 'wpmozo_filter_item_all' ); ?>>
										<a <?php $this->print_render_attribute_string( 'wpmozo_filter_item_all_anchor' ); ?>>
											<?php echo ! empty( $settings['all_images_text'] ) ? esc_html( $settings['all_images_text'] ) : esc_html__( 'All', 'wpmozo-addons-for-elementor' ); ?>
										</a>
									</li>
							<?php
							$active_class = '';
												...
												?>
													<div <?php $this->print_render_attribute_string( 'wpmozo_overlay_icon_wrap' ); ?>>
														<a <?php $this->print_render_attribute_string( 'wpmozo_overlay_lightbox_' . esc_attr( $image_id ) ); ?>>
															<?php echo esc_attr( $this->wpmozo_process_font_icon( '&#xf00e;||fa||900' ) ); ?>
														</a>
														<?php
														if ( ! empty( $image_link ) ) {
															?>
			...
			if ( '' === $show_overlay && 'lightbox' === $click_trigger ) {
				...
				<?php
			} elseif ( 'lightbox' === $title_area && 'lightbox' === $caption_area ) {
