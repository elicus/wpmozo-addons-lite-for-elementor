...
if ( $query->have_posts() ) {
	...
	while ( $query->have_posts() ) {
		$query->the_post();
		$index = $index + 1;
		$this->add_render_attribute( 'swiper_layout_item' . $index, array( 'class' => array( 'wpmozo_wrapper', 'wpomzo_ae_swiper_layout_item_' . $index, 'swiper-slide' ) ) );
		?>
			...
			<div id="wpmozo_ae_team_member_<?php echo esc_html( get_the_ID() ); ?>" class="wpmozo_ae_team_member_card"
				...
				<div class="wpmozo_team_content_wrapper">
					...
					<div class="wpmozo_ae_team_member_designation">
						<<?php echo esc_attr( $designation_heading_level ); ?>  class="wpmozo_ae_team_member_designation_heading"><?php echo esc_html( get_post_meta( get_the_ID(), 'wpmozo_ae_team_member_designation', true ) ); ?></<?php echo esc_attr( $designation_heading_level ); ?>>
					</div>
					<?php endif; ?>
